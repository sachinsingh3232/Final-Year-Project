...
// GET Request in REST API (GET Specialized Doctors)
// getDoctors() is use for getting all doctor in the database
exports.getDoctors = asyncHandler(async (req, res) => {
  ...
  // return HTTP response
  const reqDoctors=doctors.filter((doc)=>req.body.specializations.includes(doc.specialization.specialization));
  res.status(200).json({
    status: 'sucess',
    DateTime: req.requestTime,
    result: reqDoctors.length,
    data: reqDoctors,
  });
});
